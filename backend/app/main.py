...
app = FastAPI(
            title="Jurist API",
            description=(
                            "Backend API for Jurist - Albanian public procurement document management platform."
            ),
            version="1.0.0",
            lifespan=lifespan,
            docs_url="/docs",
...
# ---------------------------------------------------------------------------
# Routers — registered with BOTH /api prefix AND no prefix for test compat
# ---------------------------------------------------------------------------
from app.api.routes import auth, companies, folders, documents, procedures, analyses, matching, admin, chat  # noqa: E402

API_PREFIX = "/api"

# With /api prefix (for frontend Vite proxy + production)
...
# Without /api prefix (for direct test script access: curl localhost:8000/auth/login)
