...
export function ChatWidget() {
  ...
  const sendMessage = async (text: string) => {
    ...
    setMessages(prev => [...prev, { id: assistantId, role: 'assistant', content: '' }])
          ...
          try {
      const apiUrl = (import.meta.env.VITE_API_URL || 'http://localhost:8000') + '/api/chat/message'
      ...
      setMessages(prev => prev.map(m =>
        m.id === assistantId
          ? { ...m, content: 'Gabim ne lidhje. Provoni serish.' }
          : m
      ))
} finally {
      setLoading(false)
...
{procedureId && (
                <div className="text-indigo-200 text-xs mt-0.5">📋 Procedura #{procedureId.slice(0, 8)}</div>
              )}
{companyId && !procedureId && (
                <div className="text-indigo-200 text-xs mt-0.5">🏢 Kompania #{companyId.slice(0, 8)}</div>
              )}
